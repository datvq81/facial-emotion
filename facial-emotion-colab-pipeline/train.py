...
from emotion_model import EmotionCNN, save_checkpoint
...
def build_loaders(args: argparse.Namespace) -> tuple[DataLoader, DataLoader, list[str]]:
    ...
    sample_weights = (1.0 / counts)[np.asarray(train_set.targets)]
    sampler = WeightedRandomSampler(sample_weights, len(sample_weights), replacement=True)
    common = dict(batch_size=args.batch_size, num_workers=args.num_workers, pin_memory=True)
    train_loader = DataLoader(train_set, sampler=sampler, **common)
...
def main() -> None:
    ...
    train_loader, val_loader, labels = build_loaders(args)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if val_acc > best_accuracy:
            ...
            epochs_without_improvement = 0
            save_checkpoint(args.output, model, labels, epoch, val_acc)
